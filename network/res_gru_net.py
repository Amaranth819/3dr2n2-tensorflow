...
def build_decoder(h_f):
    with tf.variable_scope("decoder_layer0", reuse = tf.AUTO_REUSE):
        ...
        conv0_0 = layers.conv3d_layer(unpool0, n_deconv_filters[1], 3)
        conv0_1 = layers.conv3d_layer(conv0_0, n_deconv_filters[1], 3)
        output0 = unpool0 + conv0_1
        
    with tf.variable_scope("decoder_layer1", reuse = tf.AUTO_REUSE):
        unpool1 = layers.unpooling_layer(output0)
        conv1_0 = layers.conv3d_layer(unpool1, n_deconv_filters[2], 3)
        conv1_1 = layers.conv3d_layer(conv1_0, n_deconv_filters[2], 3)
        output1 = unpool1 + conv1_1
        
    with tf.variable_scope("decoder_layer2", reuse = tf.AUTO_REUSE):
        unpool2 = layers.unpooling_layer(output1)
        shortcut2 = layers.conv3d_layer(unpool2, n_deconv_filters[3], 1)
        conv2_0 = layers.conv3d_layer(unpool2, n_deconv_filters[3], 3)
        conv2_1 = layers.conv3d_layer(conv2_0, n_deconv_filters[3], 3)
        output2 = shortcut2 + conv2_1
        
    with tf.variable_scope("decoder_layer3", reuse = tf.AUTO_REUSE):
        conv3_0 = layers.conv3d_layer(output2, n_deconv_filters[4], 3)
        conv3_1 = layers.conv3d_layer(conv3_0, n_deconv_filters[4], 3)
        conv3_2 = layers.conv3d_layer(conv3_0, n_deconv_filters[4], 3)
        output3 = conv3_1 + conv3_2
        
    with tf.variable_scope("decoder_layer4", reuse = tf.AUTO_REUSE):
        output4 = layers.conv3d_layer(output3, n_deconv_filters[5], 3)
